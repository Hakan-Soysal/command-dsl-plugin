...
- "Kime (rol) **ve hangi DURUMDA** görünsün?" → `visible-when: <ifade>` — izinli kökler
  currentUser·session·ekran-param·state/derived·ekran-kaydı (detail/value adı)·row (rol + client-state İFADE EDİLEBİLİR). (+ "güvenlik değil UX".)
  ...
  **Kalıp:** eylemi, koşullandığı kaydı yükleyen **list/detail bileşeninin GÖVDESİNE** koy → satır-aksiyonu olur →
  ...
  `action X { visible-when: order.status = 'pending' }` (Karar #51 Fork B; ad = alias ?? op-adı, `activate`-hedefi gibi). **`as <alias>` rezerve bir kökü (session/currentUser/row/ekran-param/state) GÖLGELEYEMEZ** — çakışırsa validator error verir (bkz. reference §6 alias-gölgeleme).
...
3. **Sınır-devri (köprü süpürmesi):** backend'in modellediği her fact UI'da bir karşılık aldı mı?
   ...
   - **Köprüsü OLMAYAN — backend-hassas (tech `@sensitivity`/`@crypto`→maske):** bu %100
     **tech-driven** kalır; frontend re-declare ETMEZ (`@ui.hidden` ≠ PII-maske —
     ...
     bilgi orada durmalı; `#`'e ÇÖZÜM diye güvenme. `@ui.*` bunun yerine geçmez (ayrı concern).
