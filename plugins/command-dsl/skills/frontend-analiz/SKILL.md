...
sunum yetenekleri (offline mekaniği, sync-delta, tazeleme, validation, confirm, visible-when…)
sorulmazsa sessizce yok sayılır — doğrulayıcı bunu (entry/handler-tamlığı DIŞINDA) YAKALAMAZ.
Dinlerken `references/frontend-dsl-reference.md` **Yetenek Envanteri**nin "sinyal" kolonunu tara;
eşleşmeyi aday-soru olarak kuyruğa al (hibrit onay ile toplu sor). Emit'ten önce **★** satırlarını
süpür → aşağıda "Emit" (Pre-Emit Gate, ÇİFT-SIFIR).
...
- "Kime (rol) **ve hangi DURUMDA** görünsün?" → `visible-when: <ifade>` — izinli kökler
  currentUser·session·ekran-param·state/derived·row (rol + client-state İFADE EDİLEBİLİR). (+ "güvenlik değil UX".)
  ✅ **State-geçiş eylemleri** (Pause/Resume/Cancel/Delete…) çoğu zaman SUNUCU entity-durumuna bağlıdır
  ("Duraklat yalnız kampanya 'çalışıyor'ken", "İptal yalnız sipariş 'pending'ken") — ve bu **BUGÜN ifade edilir**.
  **Kalıp:** eylemi, koşullandığı kaydı yükleyen **list/detail bileşeninin GÖVDESİNE** koy → satır-aksiyonu olur →
  kaydın durumu `row`-öğe-bağlamıyla okunur: `detail GetOrder(id: …) { action CancelOrder { visible-when: row.status = 'pending' } }`
  (tek `=`; #38 `row.*` = öğe-bağlamı, list-satırı VE detail tek-kayıt). Manifest eylemi kaydın altına nest'ler (semantik doğru).
  Durum client-state/derived'daysa aynı biçimde `<state-adı>` ile gate'le. Örnek: `references/examples/orders.fcdsl` CancelOrder.
  ✅ Eylem, koşullandığı kaydın bileşen-gövdesinde DEĞİL de **ayrı region/ekran-seviyesinde** ise ("ekranın data'sı") →
  ekrandaki **detail/value** kaydına ADIYLA atıf yapılır: `detail GetOrder(…) as order` + başka region'da
  `action X { visible-when: order.status = 'pending' }` (Karar #51 Fork B; ad = alias ?? op-adı, `activate`-hedefi gibi).
  `list` HARİÇ (çok-satırlı → tek-kayıt yok; satır için gövde-içi `row`). **Tercih:** mümkünse gövde-içi (Fork A — manifest
  daha temiz nest'ler); ayrı yerleşim GEREKİYORSA ekran-kaydı referansı (Fork B).
...
- İfade path-kökleri kapalı küme: `session.*` · `currentUser.*` · ekran-param ·
  state/derived adları · `row.*` (öğe-bağlamı: liste-satırı VE detail tek-kayıt) · literal. Başkası error.
...
1. **Sessiz-eksik (★ süpürmesi):** `references/frontend-dsl-reference.md` **Yetenek Envanteri**nin
   **★** satırlarını gez; sorulmamış her ★ için ya örtük kapandığını göster, ya tek doğrulama
   sorusu sor ("Bu ekran çevrimdışı çalışmalı mı? / Bu liste büyüyebilir mi? / Bu silme onay
   istesin mi?"). Hiçbir ★'ı sessizce atlama.
2. **Sessiz-yanlış (teşhir):** sessiz **varsayılanlar** da 0-error geçer — özellikle **offline
   mekaniği** (`delivery` + op-bazı `cache/queue/remote`). Seçtiğini sessiz emit etme: "Uygulamayı
   remote-only varsaydım / bu listeyi cache'ledim — doğru mu?" diye açıkça onaylat.
3. **Sınır-devri (köprü süpürmesi):** backend'in modellediği her fact UI'da bir karşılık aldı mı?
   ...
   - **Köprüsü OLMAYAN** (tech `@sensitivity`→maske — bu bundle'da `@ui.*` YOK, snapshot `e5764dfe0cec`):
     UI bu snapshot'ta ifade EDEMİYOR. `#` yorum yalnız kaynak-içi geliştirici hatırlatmasıdır,
     **makinece-taşınmaz** → `#`'e ÇÖZÜM diye güvenme; gerçek taşıyıcı **ÜSTAKIŞTIR** (tech
     `@sensitivity`), bilgi orada durmalı. (Snapshot ilerleyip `@ui.*` girince bu satır
     "köprülü/uyumla"ya döner.)
4. **CRUD-okuma tamlığı:** kullanıcının OLUŞTURDUĞU/YAZDIĞI her entity için bir geri-GÖRME yüzeyi
   (görüntüleme/liste ekranı ya da region) var mı? Bir `create`/write eylemi ürettiğini hiç okutmuyorsa →
   **kör-oluşturma** (kullanıcı eklediğini geri göremez). Çözüm: (a) upstream'de liste/get-query op'u VARSA
   onu bir ekrana surface et (op-kapsama zaten uyarır); (b) YOKSA business'a geri-besle — "bu entity
   listelenebilmeli mi?" aday-sorusu. Frontend okuma-op'u UYDURMAZ (büyü yok) — okuma yüzeyi upstream'de doğar.

**Kalan warning = çözülmemiş soru (üçüncü hâl — error değil, sessiz-eksik değil).** Warning'i skill
KENDİ uydurduğu düzeltmeyle kapatamaz — çözüm **authored**'dır (büyü yok). Meşru kapanış: (a) sor →
düzelt, (b) gerekçeli kabul (authored divergence), (c) yanlış-pozitif göster. Sessiz auto-fix / sessiz
geçiştirme YASAK.
