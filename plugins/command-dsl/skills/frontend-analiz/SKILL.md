...
- "Kime (rol) **ve hangi client-DURUMDA** görünsün?" → `visible-when: <ifade>` — izinli kökler
  currentUser·session·ekran-param·state/derived·row (rol + client-state İFADE EDİLEBİLİR). (+ "güvenlik değil UX".)
  ⚠ **State-geçiş eylemleri** (Pause/Resume/Submit/Delete…) çoğu zaman SUNUCU entity-durumuna bağlıdır
  ("Duraklat yalnız kampanya 'çalışıyor'ken görünsün") — ama `campaign.status` visible-when path-kökü DEĞİL →
  bu şu an **ifade EDİLEMEZ**. Sessizce atlama: ihtiyacı `note` ile yakala + üstakışa taşı; bu bir
  **grammar-pilot adayı** (Faz-3: entity-durumunu visible-when kapsamına al). Durum client-state/derived'daysa
  visible-when ile gate'le.
